...
const DashboardPage = async ({ searchParams }: Props) => {
  ...
  if (user.userDetails.Status === "S1" || user.userDetails.Status === "S3") {
    ...
    redirect("/question/2");
  } else if (user.userDetails.Status === "F") {
    redirect("/finish");
  }
  ...
  return (
    <div className="flex flex-col px-3 sm:px-16 lg:px-32 py-3 sm:py-10 gap-4">
      ...
      {searchParams && searchParams.error === "week-not-started" && (
        <div className="bg-red-300 flex items-center justify-center bg-opacity-40 text-red-900 p-3 rounded-md gap-3">
          <MessageCircleWarningIcon />
          <span>
            Hafta henüz başlamadı. <b>{remainingDay} gün</b> sonra tekrar
            deneyiniz.
          </span>
        </div>
      )}
